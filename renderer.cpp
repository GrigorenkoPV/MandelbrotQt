...
Renderer::pixel_t Renderer::renderPointInRGB(qreal x0, qreal y0, unsigned max_iterations,
                                             qreal threshold) {
  static constexpr pixel_t palette[] = {
      0x19071a, 0x09012f, 0x040449, 0x000764, 0x0c2c8a, 0x1852b1, 0x397dd1, 0x86b5e5,
      0xd3ecf8, 0xf1e9bf, 0xf8c95f, 0xffaa00, 0xcc8000, 0x995700, 0x6a3403, 0x421e0f,
  };
...
std::ostream& operator<<(std::ostream& os, RenderingJob const& job) {
  return os << job.canvas_size.width() << 'x' << job.canvas_size.height()
            << ", threshold=" << job.threshold << ", iterations=" << job.max_iterations;
}
#endif
...
void Renderer::doJob(const RenderingJob& job) {
  ...
  for (auto square_side = start_square_side; square_side != 0; square_side /= 4) {
#ifndef NDEBUG
    auto t_start = std::chrono::high_resolution_clock::now();
#endif
    std::unique_ptr<void, decltype(&operator_delete)> buffer_owner(
        operator new (line_size* height, std::align_val_t{alignof(pixel_t)}), operator_delete);
    ...
    QtConcurrent::map(etalon_rows, [&](unsigned y) {
      try {
        ...
        for (unsigned x = 0; x < width;) {
          ...
          auto const color = renderPointInRGB(x0, y0, job.max_iterations, job.threshold);
    ...
    std::cerr << "Rendered " << job << " with " << square_side - 1 << " duplicates in "
              << std::chrono::duration<double, std::milli>(t_end - t_start).count() << "ms"
              << std::endl;
#endif
    emit sendImage(QImage(static_cast<uchar*>(buffer_owner.release()), job.canvas_size.width(),
                          job.canvas_size.height(), IMAGE_FORMAT, operator_delete));
    debt = 0;
  }
}
